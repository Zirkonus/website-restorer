...
class ReloadPreviews extends Job implements ShouldQueue
{
     ...
     * @var string
     */
    const DEFAULT_SCREENSHOT_WIDTH = 1024;
    ...
    protected function saveScreenshots()
    {
        foreach($this->project->versions as $version) {
            ...
            $path = 'domains/user_' . $this->project->user_id . '/project_' . $this->project->id .
                '/version_' . $version->id . '/'  . static::DEFAULT_PREVIEW_NAME;
    ...
    protected function previewResize($storage_path)
    {
        if (!Storage::exists($storage_path . static::DEFAULT_PREVIEW_NAME)) {
            Image::make(storage_path($storage_path) . static::DEFAULT_PREVIEW_NAME)->fit(400, 400, function ($constraint) {
                $constraint->upsize();
